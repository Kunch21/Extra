...
// query data galeri_kkr berdasarkan id
$gkkr = query("SELECT * FROM galeri_kkr WHERE id_glri = $id")[0];

// cek apakah tombil submit sudah ditekan atau belum
if( isset($_POST["submit"]) ) {

    // cek apakah data berhasil ditambahkan atau tidak
    if ( updategkkr($_POST) > 0 ) {
        echo "
            <script>
...
<body>
    ...
    <div class="divform">
        <h1>Update Data Galeri KKR</h1>
        <form action="" method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?= $gkkr["id_glri"]; ?>">
            <input type="hidden" name="oldimage" value="<?= $gkkr["image"]; ?>">
            <div class="mb-3">
                <label for="image" class="form-label">Image</label>
                <img src="photo/<?= $gkkr["image"]; ?>" width="50">
